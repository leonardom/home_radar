...
export default function FiltersPage() {
  ...
  const loadFilters = async () => {
    ...
    try {
      const response = await withAuthenticatedSession((token) => getFilters(token));
      setFilters(response.items);
      setError("");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Failed to load filters");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      propertyType:
        String(formData.get("propertyType") ?? "").trim() === ""
          ? undefined
          : (String(formData.get("propertyType")) as FilterItem["propertyType"]),
      priceMin:
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Failed to save filter");
    }
  };
  ...
  const onDelete = async (filterId: string) => {
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Failed to remove filter");
    }
  };
  ...
  return (
    <div className="stack-lg">
      <section className="hero compact">
        ...
        <p>Adjust your criteria and keep alerts aligned with your target market.</p>
      </section>
      ...
      <section className="panel stack-sm">
        ...
        {!isLoading && filters.length === 0 && <p className="muted">No filters created yet.</p>}
        <ul className="list">
          {filters.map((filter) => (
            <li key={filter.id} className="saved-row">
              <div>
                <p className="saved-title">
                  {filter.location ?? "Any location"} · {filter.propertyType ?? "Any type"}
                </p>
                <p className="muted">
                  Price {filter.priceMin ?? 0} - {filter.priceMax ?? "any"} · Bedrooms min {" "}
                  {filter.bedroomsMin ?? 0}
                </p>
              </div>
